"""
{agent_name} — PydanticAI Agent on ZyndAI Network
...
def create_agent():
    ...
    agent = Agent(
        model,
        system_prompt="You are {agent_name}, a helpful AI assistant.",
        result_type=str,
    )
...
if __name__ == "__main__":
    agent_config = AgentConfig(
        name="{agent_name}",
        description="{agent_name} — a PydanticAI agent on the ZyndAI network.",
        capabilities={{
            "ai": ["nlp", "pydantic_ai"],
            "protocols": ["http"],
        }},
    ...
    print(f"\n{agent_name} is running (PydanticAI)")
